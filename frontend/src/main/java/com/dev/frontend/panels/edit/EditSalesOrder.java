...
import com.dev.frontend.dto.OrderLinesDto;
...
public class EditSalesOrder extends EditContentPanel {
	...
	public boolean bindToGUI(Object o) {
		...
		if (o != null) {
			...
			txtOrderNum.setText(saleOrderDto.getOrderNumber());

			ComboBoxItem comboBoxItem = new ComboBoxItem(saleOrderDto.getCustomer().getCode(),saleOrderDto.getCustomer().getName());
			txtCustomer.setSelectedItem(comboBoxItem);

			saleOrderDto.setTotalPrice(saleOrderDto.getTotalPrice());

			int quantity = 0;
			if (saleOrderDto.getOrderLines() != null && saleOrderDto.getOrderLines().size() > 0) {
				for (OrderLinesDto orderLines : saleOrderDto.getOrderLines()) {
					defaultTableModel.addRow(new String[] { orderLines.getProduct().getCode(), "" + orderLines.getQuantity(),
							"" + Constants.df.format(orderLines.getProduct().getPrice()),
							"" + (Constants.df.format(orderLines.getQuantity() * orderLines.getProduct().getPrice())) });
					quantity += orderLines.getQuantity();
				}
			}
			txtTotalPrice.setText(Constants.df.format(saleOrderDto.getTotalPrice()));
	...
	public Object guiToObject() {
		...
		String order = txtOrderNum.getText();
		if(((ComboBoxItem) txtCustomer.getSelectedItem()) == null){
			return null;
		}
		String customerId = ((ComboBoxItem) txtCustomer.getSelectedItem()).getKey();
		String totalPrice = txtTotalPrice.getText();
		int rowCount = defaultTableModel.getRowCount();

		if (rowCount > 0) {
			saleOrderDto = new SaleOrderDto();
			saleOrderDto.setCustomer(new CustomerDto(customerId));
			saleOrderDto.setTotalPrice(Double.parseDouble(totalPrice));
			saleOrderDto.setOrderNumber(order);
			for (int i = 0; i < rowCount; i++) {
				saleOrderDto.getOrderLines().add(new OrderLinesDto(new ProductDto(defaultTableModel.getValueAt(i, 0) + ""),
						Long.parseLong(defaultTableModel.getValueAt(i, 1) + "")));
			}
		}
		return saleOrderDto;
	}
